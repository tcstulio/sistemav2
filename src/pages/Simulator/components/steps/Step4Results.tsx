...
const Step4Results: React.FC<Step4Props> = ({ data, setData, results, onSave }) => {
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-bottom fade-in duration-500">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-indigo-100 dark:border-slate-700 shadow-sm">
                <div className="flex items-center gap-2 mb-6">
                    <Settings2 className="text-indigo-600" size={18} />
                    <h3 className="font-bold text-slate-700 dark:text-slate-200 text-sm uppercase">Sensibilidade de Cenário (Ajuste Rápido)</h3>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div className="space-y-3">
                        <div className="flex justify-between items-end">
                            <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase">Público</label>
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-end">
                            <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase">Ticket Médio</label>
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-end">
                            <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase">Bar / Pax</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border-2 border-indigo-50 dark:border-slate-700 overflow-hidden flex flex-col">
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                        <div className="mb-6 text-center">
                            <span className="text-xs text-slate-400 dark:text-slate-500 uppercase font-bold">Lucro Operacional</span>
                            <div className={`text-4xl font-black ${venueProfitClass} tracking-tight`}>{money(results.venue.profit)}</div>
                            <div className="text-sm text-slate-500 dark:text-slate-400 mt-1">Margem: {percent(results.venue.roi)} (ROI)</div>
                        </div>
                        <div className="space-y-2 flex-1">
                            {results.venue.items.map((item: any, i: number) => (
                                <div key={i} className={`flex justify-between text-sm py-2 border-b border-slate-50 dark:border-slate-700 last:border-0 ${item.type === 'neutral' ? 'italic opacity-70' : ''}`}>
                                    <span className={`${item.type === 'neutral' ? 'text-slate-400 dark:text-slate-500' : 'text-slate-600 dark:text-slate-300'}`}>{item.label}</span>
                                    <span className={`font-mono font-bold ${item.type === 'neutral' ? 'text-slate-400 dark:text-slate-500' : item.value >= 0 ? 'text-indigo-600' : 'text-rose-500'}`}>{item.value > 0 ? '+' : ''}{money(item.value)}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="space-y-4">
                    {data.partners.length === 0 ? <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border-2 border-emerald-50 dark:border-slate-700 overflow-hidden flex flex-col h-full items-center justify-center p-8 text-center text-slate-400 dark:text-slate-500"><p>Sem parceiros externos.</p><p className="text-xs">Produção 100% Casa.</p></div> :
                        (Object.values(results.partnersResults) as FinancialResult[]).map((pRes, idx) => (
                            <div key={idx} className="bg-white dark:bg-slate-800 rounded-3xl shadow-lg border border-emerald-100 dark:border-slate-700 overflow-hidden flex flex-col">
                                ...
                                <div className="p-4 bg-emerald-50/30 dark:bg-emerald-900/10">
                                    <div className="space-y-1">
                                        {pRes.items.map((item, i) => (
                                            <div key={i} className={`flex justify-between text-xs py-1 border-b border-emerald-100/50 dark:border-slate-700 last:border-0 ${item.type === 'neutral' ? 'italic opacity-70' : ''}`}>
                                                <span className={`${item.type === 'neutral' ? 'text-slate-500 dark:text-slate-400' : 'text-slate-600 dark:text-slate-300'}`}>{item.label}</span>
                                                <span className={`font-mono font-bold ${item.type === 'neutral' ? 'text-slate-500 dark:text-slate-400' : item.value >= 0 ? 'text-emerald-600' : 'text-rose-500'}`}>{item.value > 0 ? '+' : ''}{money(item.value)}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
