...
export const MonthlyReport: React.FC = () => {
    ...
    const { data: supplierPayments } = useSupplierPayments(config);
    const { data: salaries } = useSalaryPayments(config);
    const { data: taxes } = useSocialContributionPayments(config);
    const { data: vat } = useVATPayments(config);

    const { data: proposals } = useProposals(config);
    const { data: orders } = useOrders(config);

    const { data: users } = useUsers(config);
    const { data: leaves } = useLeaveRequests(config);
    ...
    return (
        <div className="p-6 bg-gray-50 h-full overflow-y-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800">Relatório Mensal</h1>
                    <p className="text-gray-500">Análise de desempenho e indicadores chave</p>
                </div>
                ...
                <div className="flex flex-wrap gap-2 items-center bg-white p-2 rounded-lg shadow-sm border border-gray-200">
                    <select
                        ...
                        className="p-2 border-r border-gray-100 bg-transparent focus:outline-none text-gray-700 font-medium cursor-pointer"
                    >
                        {MONTHS.map((m, i) => (
                    ...
                    <select
                        ...
                        className="p-2 bg-transparent focus:outline-none text-gray-700 font-medium cursor-pointer"
                    >
...
const Card = ({ title, value, subValue, color }: any) => {
    const colors: any = {
    ...
    return (
        <div className={`p-6 rounded-lg border ${colors[color] || 'bg-white border-gray-200'}`}>
            <h3 className="text-xs font-semibold uppercase opacity-70 mb-2">{title}</h3>
