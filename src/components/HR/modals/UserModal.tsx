...
export const UserModal: React.FC<UserModalProps> = ({
    ...
    const [userForm, setUserForm] = useState<Partial<DolibarrUser> & { supervisor_id?: string }>({ login: '', firstname: '', lastname: '', email: '', job: '', office_phone: '', phone_mobile: '', supervisor_id: '' });
        ...
        if (isOpen) {
            if (userToEdit) {
                setUserForm({
                    ...
                    job: userToEdit.job || '',
                    office_phone: userToEdit.office_phone || '',
                    phone_mobile: userToEdit.phone_mobile || '',
                ...
                setUserForm({ login: '', firstname: '', lastname: '', email: '', job: '', office_phone: '', phone_mobile: '', supervisor_id: '' });
            }
        }
    ...
    const handleUserSubmit = async (e: React.FormEvent) => {
        ...
        if (payload.phone_mobile !== undefined) {
            payload.user_mobile = payload.phone_mobile;
            delete payload.phone_mobile;
        }
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 overflow-hidden">
            <div className="bg-white dark:bg-slate-900 rounded-xl p-6 w-full max-w-md max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95">
                <h3 className="font-bold text-lg mb-4 dark:text-white flex-none">{userToEdit ? 'Editar Usuário' : 'Novo Usuário'}</h3>
                <div className="space-y-3 overflow-y-auto flex-1 min-h-0 pr-1">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <input className="w-full p-2 border rounded dark:bg-slate-800 dark:border-slate-700 dark:text-white" placeholder="Telefone Fixo" value={userForm.office_phone || ''} onChange={e => setUserForm({ ...userForm, office_phone: e.target.value })} />
                        <input className="w-full p-2 border rounded dark:bg-slate-800 dark:border-slate-700 dark:text-white" placeholder="Celular" value={userForm.phone_mobile || ''} onChange={e => setUserForm({ ...userForm, phone_mobile: e.target.value })} />
                    </div>
                ...
                <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex-none">
