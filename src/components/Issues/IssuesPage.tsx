...
import { AlertCircle, Bug, Sparkles, Shield, Wrench, TestTube, GitMerge, Loader2, Eye, CheckCircle, XCircle, RotateCcw, MessageSquare, Trash2, Pencil, Terminal, ExternalLink, Search, Tag, CircleDot, Play, Clock, RefreshCw, ThumbsUp, Star, ShieldOff, Plus, Filter, LayoutGrid, List, GripVertical } from 'lucide-react';
...
const SortableMiniCard: React.FC<{
    ...
    const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: task.issueNumber });
    const style = {
...
const MiniCard: React.FC<{
    ...
    queuePosition?: number;
    dragHandleProps?: any;
}> = ({ task, onAction, onReview, onEdit, onDelete, onConsole, isAdmin, queuePosition, dragHandleProps }) => {
    ...
    return (
        <div className={`p-3 rounded-lg border ${isActive ? 'border-blue-300 dark:border-blue-700 bg-blue-50/50 dark:bg-blue-900/10' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50'} transition-all hover:shadow-sm`}>
            <div className="flex items-center gap-2 mb-1">
                {dragHandleProps && (
                    <span className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-slate-600" {...dragHandleProps}>
...
const IssuesPage: React.FC = () => {
    ...
    const [tab, setTab] = useState<'issues' | 'tasks' | 'stats'>(() => {
        const urlTab = searchParams.get('tab');
        if (urlTab === 'tasks' || urlTab === 'stats') return urlTab;
        return 'issues';
    });
    ...
    useEffect(() => {
        const urlTab = searchParams.get('tab');
        if (urlTab === 'tasks' && tab !== 'tasks') setTab('tasks');
    }, [searchParams]);

    const handleTabChange = (v: string) => {
        setTab(v as any);
        setSearchParams(v === 'issues' ? {} : { tab: v }, { replace: true });
    };
    ...
    const filteredTasks = useMemo(() => {
        ...
        if (taskStatusFilter !== 'all') result = result.filter(t => t.status === taskStatusFilter);
        if (taskSearch.trim()) {
        ...
        return result;
    }, [tasks, taskTab, taskStatusFilter, taskSearch]);
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        const oldIndex = queueOrder.findIndex(t => t.issueNumber === active.id);
        const newIndex = queueOrder.findIndex(t => t.issueNumber === over.id);
        ...
        <PageLayout title="Issues & Tasks">
            ...
            {tab === 'tasks' && (
                <div className="mt-6 space-y-4">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Button variant="ghost" size="sm" icon={<RefreshCw size={14} />} onClick={loadTasks}>Atualizar</Button>
                    </div>
                            ...
                            {viewMode === 'pipeline' ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                    {PIPELINE_COLUMNS.map(col => {
                                        ...
                                        const isQueueColumn = col.key === 'queue';
                                        const queueTaskIds = queueOrder.map(t => t.issueNumber);
                                        ...
                                        return (
                                            <div key={col.key} className="flex flex-col">
                                                <div className="flex items-center gap-2 mb-2 px-1">
                                                    ...
                                                    {isQueueColumn && isAdmin && colTasks.length > 1 && (
                                                        <span className="text-[9px] text-slate-400">arraste p/ reordenar</span>
                                                    )}
                                                </div>
                                                <div className="flex flex-col gap-2 min-h-[100px]">
                                                    ...
                                                    {isQueueColumn && isAdmin && colTasks.length > 1 ? (
                                                        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                                                            <SortableContext items={queueTaskIds} strategy={verticalListSortingStrategy}>
                                                                {colTasks.map(task => (
                                                                    <SortableMiniCard
                                                                        ...
                                                                        onReview={openReview}
                                                                        onEdit={t => { setEditTask(t); setEditTitle(t.title); setEditBody(t.body); }}
                                        ...
                                        <Card>
                                            <div className="text-center py-8 text-slate-400">
                                                ...
                                                <p className="text-xs mt-1">
                                                    {taskSearch || taskStatusFilter !== 'all' ? 'Tente ajustar os filtros' : isAdmin ? <button onClick={() => setShowCreate(true)} className="text-indigo-500 hover:underline">Criar uma nova task</button> : 'Aguardando issues com label "opencode-task"'}
                                                </p>
                                            </div>
                                        </Card>
                                    )}
            ...
            {reviewTask && (diffLoading ? (
                ...
                <DiffViewer diff={diffText} judgeScore={reviewTask.judgeScore} judgeReview={reviewTask.judgeReview} prUrl={reviewTask.prUrl} onClose={() => setReviewTask(null)}
                    onMerge={async () => { if (!isAdmin) { toast.error('Apenas administradores.'); return; } await TaskService.merge(reviewTask.issueNumber); setReviewTask(null); toast.success('Merged!'); loadTasks(); }}
            ...
            {editTask && (
                <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setEditTask(null)}>
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-lg space-y-4" onClick={e => e.stopPropagation()}>
                        ...
                        <div><label className="text-xs font-medium text-slate-500">Título</label><input type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} className="w-full mt-1 text-sm px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
                        <div><label className="text-xs font-medium text-slate-500">Descrição</label><textarea value={editBody} onChange={e => setEditBody(e.target.value)} rows={8} className="w-full mt-1 text-sm px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none resize-none" /></div>
...
const TaskCard: React.FC<{ task: Task; onAction: (a: string, t: Task, extra?: string) => void; onReview: (t: Task) => void; onEdit: (t: Task) => void; onDelete: (t: Task) => void; onConsole: (t: Task) => void; isAdmin: boolean }> = ({ task, onAction, onReview, onEdit, onDelete, onConsole, isAdmin }) => {
    ...
    return (
        <Card className="relative overflow-hidden">
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
                {isAdmin && task.status === 'pending' && <Button variant="primary" size="sm" icon={<Play size={12} />} onClick={() => onAction('start', task)}>Iniciar</Button>}
                {(task.status === 'reviewing' || task.status === 'approved') && (
                    <>
                ...
                {isAdmin && task.status === 'failed' && <Button variant="primary" size="sm" icon={<RotateCcw size={12} />} onClick={() => onAction('redo', task)}>Tentar Novamente</Button>}
                {task.status === 'merged' && <span className="text-xs text-emerald-600 flex items-center gap-1"><CheckCircle size={12} /> Concluído</span>}
