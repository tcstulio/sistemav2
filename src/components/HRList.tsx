...
import { DolibarrUser, ExpenseReport, RecruitmentJobPosition, DolibarrConfig, Task, Project, Ticket, AppView, Candidate, LeaveRequest } from '../types';
import { Search, Plus, Filter, ArrowUp, ArrowDown, UserCheck, Plane, BarChart3, Banknote, Briefcase, Scan, Users, ArrowUpDown } from 'lucide-react';
...
const HRList: React.FC<HRListProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'team' | 'workload' | 'expenses' | 'leaves' | 'recruitment'>('team');
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 transition-colors relative">
            ...
            <ExpenseDetailModal expense={selectedExpense} onClose={() => setSelectedExpense(null)} config={config} users={users} />
            ...
            <div className="flex-1 min-h-0 flex overflow-hidden">
                {/* List Side */}
                <div className={`flex-1 overflow-y-auto p-4 md:p-6 ${(selectedUser || viewingCandidates) ? 'hidden lg:block lg:w-1/3 xl:w-1/4 border-r border-slate-200 dark:border-slate-800' : 'w-full'}`}>
                    {activeTab === 'team' && (
                        <TeamTab
                            users={users}
                ...
                {/* Detail View Side */}
                <div className={`flex-1 bg-white dark:bg-slate-900 flex flex-col ${(selectedUser || viewingCandidates) ? 'block absolute inset-0 z-20 lg:static lg:inset-auto' : 'hidden lg:flex lg:items-center lg:justify-center'}`}>
                    {selectedUser ? (
                        <UserDetail
                            user={selectedUser}
                            userTasks={userTasks}
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-slate-400">
                            ...
                            {activeTab === 'expenses' && <><Banknote size={48} className="mb-4 opacity-50" /><p>Selecione uma despesa</p></>}
