...
export const CreateMOModal: React.FC<CreateMOModalProps> = ({ isOpen, onClose, config, products, projects, onSuccess, initialForm }) => {
    ...
    const handleCreateMo = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!moForm.product_to_produce_id) return;
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-lg p-6">
                <div className="flex justify-between mb-4">
                    <h3 className="font-bold text-lg dark:text-white">Nova Ordem de Produção</h3>
                ...
                <form onSubmit={handleCreateMo} className="space-y-4">
                    ...
                    <div>
                        ...
                        <select className="w-full p-2 border rounded dark:bg-slate-800 dark:border-slate-700 dark:text-white" value={moForm.product_to_produce_id} onChange={e => setMoForm({ ...moForm, product_to_produce_id: e.target.value })} required>
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                        ...
                        <button type="submit" disabled={isSubmittingMo} className="px-4 py-2 bg-indigo-600 text-white rounded flex items-center gap-2">
                            {isSubmittingMo ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />} Criar
                        </button>
                    </div>
                </form>
            </div>
