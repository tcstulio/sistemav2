    ...
    while (keepFetching && safetyCounter < 500) {
        try {
            ...
            if (page === 0 && e.message && (e.message.includes('404') || e.message.includes('Not Found'))) {
                ...
                console.warn(`[DoliGenAI] NÃ£o autorizado (401) para ${endpoint}. Parando busca.`);
                if (page === 0) throw e;
                keepFetching = false;
                ...
                console.error(`[DoliGenAI] Erro buscando ${endpoint} pÃ¡gina ${page}:`, e);
                keepFetching = false;
...
export const checkConnection = async (apiUrl: string, apiKey: string) => {
    ...
    if (!response.ok) {
        throw new Error(`Falha na conexÃ£o: ${response.status} ${response.statusText}`);
    }
    return true;
};
...
export const fetchDictionary = async (config: DolibarrConfig, dictionaryType: string): Promise<DolibarrDictionary[]> => {
    ...
    try {
        ...
        console.warn(`Falha ao buscar dicionÃ¡rio ${dictionaryType}`, e);
        return [];
    }
};
...
export const login = async (login: string, password: string): Promise<{ token: string, entity: string, message: string, apiKey?: string, user?: DolibarrUser }> => {
    try {
        ...
        if (contentType && contentType.indexOf("application/json") !== -1) {
            ...
            throw new Error(`Erro no Servidor (${response.status}): Resposta invÃ¡lida.`);
        }
    } catch (error: any) {
