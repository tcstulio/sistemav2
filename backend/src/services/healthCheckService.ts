...
async function defaultCheckWhatsapp(): Promise<WhatsappCheckResult> {
    // Lazy require para não carregar whatsapp-web.js no import do módulo.
    const { sessionService } = require('./legacy/sessionService');
...
function isBankConfigured(storeKey: 'inter' | 'itau', envClientId: string): boolean {
    try {
        const { bankingCredentialsStore } = require('./bankingCredentialsStore');
...
async function defaultCheckBancoInter(): Promise<BaseCheck> {
    if (!isBankConfigured('inter', config.interClientId)) return { status: 'not_configured' };
    const { interApiService } = require('./interApiService');
...
async function defaultCheckBancoItau(): Promise<BaseCheck> {
    if (!isBankConfigured('itau', config.itauClientId)) return { status: 'not_configured' };
    const { itauApiService } = require('./itauApiService');
...
export async function checkAll(deps?: Partial<HealthCheckDeps>): Promise<HealthReport> {
    ...
    try {
        ...
        const down = (label: string): BaseCheck => ({ status: 'down', error: 'global timeout' });
        checks = {
            dolibarr: down('dolibarr'),
            whatsapp: { status: 'down', session: 'unknown', error: 'global timeout' },
            bancoInter: down('bancoInter'),
            bancoItau: down('bancoItau'),
