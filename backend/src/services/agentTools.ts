...
export const TOOLS_PROMPT = `
        ...
        45. prepare_create_event(label, date_start, date_end?, type_code?, description?) - Rascunho de evento na agenda. date_start/date_end no formato "YYYY-MM-DDTHH:mm". type_code deve ser UM dos códigos reais da CoolGroove (NÃO use os códigos baunilha do Dolibarr): ta_vt (visita técnica), ta_duvidas (prospecto / dúvidas), Contratado (evento confirmado, aberto ao público), Contratado_externa (evento externo, equipe vai ao local), Contratado_fechadoaopublico (evento fechado), agenda_ensaios (ensaio), montagem_agenda (montagem). Escolha o que melhor descreve o tipo de compromisso.
        46. prepare_edit_event(id, label?, date_start?, date_end?, description?, percentage?) - Prepara EDIÇÃO de um evento. Ache o id antes com list_events. date_start/date_end no formato "YYYY-MM-DDTHH:mm". percentage: 0-100 (progresso). type_code NÃO é editável (definido na criação). Tipos válidos da CoolGroove para referência: ta_vt, ta_duvidas, Contratado, Contratado_externa, Contratado_fechadoaopublico, agenda_ensaios, montagem_agenda.
