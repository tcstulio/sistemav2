...
import { reportToolMap } from '../agent/tools/registry';
...
async function executeToolInner(tool: string, args: any): Promise<string> {
    switch (tool) {
        ...
        case 'get_report_html': {
            return reportToolMap[tool].execute(args);
        }
