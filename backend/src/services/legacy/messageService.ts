...
export class MessageService {
    ...
    private getClient(sessionId: string) {
        ...
        const status = sessionService.getStatus(sessionId);
        if (status !== 'WORKING' && status !== 'STARTING') throw new Error(`Session ${sessionId} not ready`);
        return client;
    }
    ...
    async getChats(sessionId: string) {
        ...
        try {
            ...
            log.error(`[${sessionId}] getChats error:`, e);
            return [];
        }
    }
    ...
    async getMessages(sessionId: string, chatId: string, limit: number = 50) {
        ...
        try {
            ...
            log.error(`[${sessionId}] getMessages error:`, e);
            return [];
        }
    }
