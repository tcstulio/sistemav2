...
describe('healthCheckService - httpStatusFor', () => {
    it('retorna 503 para status "down"', () => {
        expect(httpStatusFor({ status: 'down' })).toBe(503);
        expect(httpStatusFor({ status: 'down' }, false)).toBe(503);
    });

    it('retorna 503 para status "degraded" quando failOnDegraded=true (default)', () => {
        expect(httpStatusFor({ status: 'degraded' })).toBe(503);
        expect(httpStatusFor({ status: 'degraded' }, true)).toBe(503);
    });

    it('retorna 200 para status "degraded" quando failOnDegraded=false', () => {
        expect(httpStatusFor({ status: 'degraded' }, false)).toBe(200);
    });

    it('retorna 200 para status "ok"', () => {
        expect(httpStatusFor({ status: 'ok' })).toBe(200);
        expect(httpStatusFor({ status: 'ok' }, false)).toBe(200);
    });
});
