...
vi.mock('../../services/llmQuotaState', () => ({
    ...
    quotaStatus: vi.fn(() => ({ exhausted: quotaState.exhausted, since: null, reason: '' })),
}));
...
function mockExecFileOk() {
    vi.mocked(execFile as any).mockImplementation((_cmd: string, _args: any[], opts: any, cb: any) => {
        if (typeof opts === 'function') { cb = opts; }
        // O `gh pr close` dispara em BACKGROUND; o callback é resolvido em próxima tick.
...
describe('#1113 — requeueTask: re-enfileirar SEM re-planejar (reusa o plano)', () => {
    ...
    it('reseta para pending, PRESERVA queuePriority/planReason e agenda com skipPlanner=true', async () => {
        ...
        // 5º argumento = opts com skipPlanner true (caminho "sem re-planejar"). 4º continua sendo o slot.
        const call = svc.scheduleExec.mock.calls[0];
        expect(call[0]).toBe(task);
        expect(call[1]).toBe('fix-6001');
        expect(call[2]).toBe('running');
        expect(call[3]).toBeDefined(); // slot
        expect(call[4]).toEqual({ skipPlanner: true });
    });

    it('com replan:true agenda SEM skipPlanner (força re-análise do Planner)', async () => {
        await svc.requeueTask(6001, { replan: true });
        const call = svc.scheduleExec.mock.calls[0];
        expect(call[4]).toEqual({ skipPlanner: false });
    });
...
describe('#1113 — requeueBatch: recuperação em LOTE (reusa plano, sem LLM)', () => {
    beforeEach(() => {
        ...
        svc.store = {
            tasks: {
                6101: { issueNumber: 6101, status: 'failed', queuePriority: 1, planReason: 'p1', branch: 'fix-6101', kind: 'task', events: [], attempts: [] } as Task,
                6102: { issueNumber: 6102, status: 'approved', queuePriority: 2, planReason: 'p2', branch: 'fix-6102', kind: 'task', events: [], attempts: [] } as Task,
                // 6103 propositalmente ausente (não encontrada)
                6104: { issueNumber: 6104, status: 'pending', kind: 'epic', events: [], attempts: [] } as Task,
                6105: { issueNumber: 6105, status: 'running', kind: 'task', events: [], attempts: [] } as Task,
            },
        };
    });
    ...
    it('re-enfileira as tasks válidas e reporta skipped (ausente/épica/em execução)', async () => {
        ...
        const call1 = svc.scheduleExec.mock.calls[0];
        expect(call1[0].issueNumber).toBe(6101);
        expect(call1[1]).toBe('fix-6101');
        expect(call1[2]).toBe('running');
        expect(call1[4]).toEqual({ skipPlanner: true });
    ...
    it('lote com 50+ tasks retorna rápido (sem disparar N chamadas LLM/paralelas)', async () => {
        // Cenário do incidente 2026-07-06: tentar re-enfileirar 57 tasks estourava timeout.
        const N = 57;
        const tasks: Record<number, Task> = {};
        for (let i = 0; i < N; i++) {
            tasks[7000 + i] = {
                issueNumber: 7000 + i, status: 'failed', queuePriority: i, planReason: `p${i}`,
                branch: `fix-${7000 + i}`, kind: 'task', events: [], attempts: [],
                feedbackHistory: [], durableFeedback: [], title: `t${i}`, body: '', labels: [],
            } as Task;
        }
        svc.store.tasks = tasks;

        const start = Date.now();
        const result = await svc.requeueBatch(Array.from({ length: N }, (_, i) => 7000 + i));
        const elapsed = Date.now() - start;

        expect(result.requeued.length).toBe(N);
        expect(result.skipped.length).toBe(0);
        expect(svc.scheduleExec).toHaveBeenCalledTimes(N);
        // Sem chamadas LLM (analyzeTask é mockado); cronometra apenas o enfileiramento.
        expect(elapsed).toBeLessThan(1000); // bem abaixo do timeout do cliente
        expect(vi.mocked(taskPlannerService.analyzeTask)).not.toHaveBeenCalled();
    });
});
...
describe('#1113 — redoTask: apenas enfileira (pending) e retorna rápido', () => {
    beforeEach(() => {
        ...
        svc.store = {
            tasks: {
                6201: { issueNumber: 6201, title: 'Bug Y', status: 'failed', branch: 'fix-6201', kind: 'task', phase: 'done', events: [], attempts: [], feedbackHistory: [] } as Task,
            },
        };
    });
    ...
    it('seta status=pending (NÃO running) e agenda a execução', async () => {
        ...
        const call = svc.scheduleExec.mock.calls[0];
        expect(call[0]).toBe(task);
        expect(call[1]).toBe('fix-6201');
        expect(call[2]).toBe('running');
        // 5º arg (opts) deve ser undefined — redo NÃO reusa plano, passa pelo Planner.
...
describe('#1113 — scheduleExec real: skipPlanner pula o Planner na chain serial', () => {
    beforeEach(() => {
        ...
        svc.withWorktreeLock = async (_label: string, _slot: any, fn: () => any) => fn();
        ...
        svc.store = {
            tasks: {
                6301: { issueNumber: 6301, title: 'Bug Z', status: 'pending', branch: 'fix-6301', kind: 'task', phase: 'exploring', events: [], attempts: [], queuePriority: 9, planReason: 'ja planejado', feedbackHistory: [] } as Task,
            },
        };
    });
    ...
    it('com skipPlanner=true: NÃO entra no Planner e vai direto à execução (reusa o plano)', async () => {
        ...
        const slot = svc.slotForTask(task);
        svc.scheduleExec(task, 'fix-6301', 'running', slot, { skipPlanner: true });
        await svc.execChains.get(slot.id);
    ...
    it('sem skipPlanner: entra no Planner (contraprova do caminho normal) e prossegue à execução', async () => {
        const task = svc.store.tasks[6301];
        const slot = svc.slotForTask(task);
        svc.scheduleExec(task, 'fix-6301', 'running', slot);
        await svc.execChains.get(slot.id);
